function result = SSA(func, dim, bound, max_iter, pop_size, r_a, p_c, p_m,val, info)
...
%position = val*(rand(pop_size, dim) * 2 * bound - bound);
position = val*(0+rand(pop_size, dim)*bound);
%position = sort(position,'descend');
...
while (iter < max_iter)
    ...
    bound_const_up = position>1;
    position = bound_const_up + (1-bound_const_up).*position;
    bound_const_d = position<0;
    position = bound_const_d.*0 + (1-bound_const_d).*position;
